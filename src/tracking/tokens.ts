...
export interface SessionStats {
  ...
  totalOutput: number;
  totalCost: number;
  totalDurationMs: number;    // sum of API call durations
  wallStartMs: number;        // session wall-clock start
  turns: number;
  byModel: Map<string, { input: number; output: number; cost: number; calls: number }>;
}
...
export class TokenTracker {
  ...
  }, provider: string, model: string, durationMs = 0): TokenUsage {
  ...
  checkBudget(): { over: boolean; warning: boolean; message: string | null } {
    if (this.budgetUsd === null) return { over: false, warning: false, message: null };
  ...
  static listPricingTable(): Array<{ key: string; input: number; output: number }> {
