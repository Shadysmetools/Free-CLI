 ...
 */
function couldBecomeToolMarker(text: string): boolean {
  const t = (text || '').replace(/^\s+/, '');
  if (!t) return true; // only whitespace so far — undecided
  return '<tool_call>'.startsWith(t) || '```'.startsWith(t.slice(0, 3));
}

type Decision = 'unknown' | 'prose' | 'tool';

export class StreamFilter {
  private acc = '';
  private decided: Decision = 'unknown';
  ...
  push(token: string): string {
    ...
    if (this.decided === 'prose') return token;
    ...
    if (couldBecomeToolMarker(this.acc)) return '';
    this.decided = 'prose';
