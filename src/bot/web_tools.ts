...
async function searchWithDuckDuckGo(query: string): Promise<ToolResult> {
  try {
    ...
    const resp = await axios.get<DuckDuckGoResponse>(url, {
      timeout: 10000,
      headers: {
...
async function searchWithBrave(query: string, apiKey: string): Promise<ToolResult> {
  try {
    const url = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=5`;
    const resp = await axios.get<BraveSearchResponse>(url, {
      timeout: 10000,
      headers: {
 ...
 */
export async function executeWebFetch(url: string, maxChars = 8000): Promise<ToolResult> {
  if (!url?.trim()) {
 ...
 */
export async function executeApiCall(args: ApiCallArgs): Promise<ToolResult> {
