...
<i>(Just type your name or nickname)</i>`);
  ...
  // ── /clear ────────────────────────────────────────────────────────────────
  {
    ...
    async handler(ctx, runtime) {
      ...
      await reply(ctx, `🗑 Chat cleared, ${escapeHtml(name)}! Your identity & settings are kept. Just start chatting again.`);
    },
  },
    ...
    async handler(ctx, runtime, args) {
      if (args.length < 2) {
        ...
        for (const [name, status] of Object.entries(envMap)) {
          lines.push(`  ${status} ${escapeHtml(name)}`);
        }
      ...
      if (!envVar) {
        await reply(ctx, `❌ Unknown provider: <code>${escapeHtml(provider)}</code>\n\nTry: openrouter, groq, google, mistral, anthropic, openai`);
        return;
      }
      ...
      await reply(ctx, `✅ <b>${escapeHtml(provider)}</b> API key set!\n\n🔒 Your message with the key was deleted for security.`);
    },
  },
