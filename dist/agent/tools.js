...
async function generatePdf(args, cwd) {
    try {
        ...
        // Author
        const authorName = args.author ?? 'knowcap-code';
        ...
        doc.fontSize(8).font('Helvetica').fillColor('#999999')
            .text('Generated by knowcap-code', margin, pageHeight - 50, { align: 'center', width: pageWidth });
        // Reset fill color
        ...
        for (let i = 1; i < totalPages; i++) {
            ...
            doc.save();
            doc.fontSize(8).font('Helvetica').fillColor('#999999')
                .text('Generated by knowcap-code', margin, footerY, { align: 'left', continued: true, width: pageWidth / 2 })
                .text(`Page ${i} of ${totalPages - 1}`, margin + pageWidth / 2, footerY, { align: 'right', width: pageWidth / 2 });
            doc.restore();
        }
        doc.end();
