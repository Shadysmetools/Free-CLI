...
exports.readDocumentContent = readDocumentContent;
exports.buildDocumentContext = buildDocumentContext;
exports.buildMediaContext = buildMediaContext;
 ...
 */
async function transcribeAudio(localPath, groqApiKey) {
    // Try Groq first (free, fast, no local install needed)
    if (groqApiKey) {
        try {
            const { transcribeViaGroq } = await Promise.resolve().then(() => __importStar(require('../whisper/transcribe')));
    ...
    // Try local Whisper
    try {
        const { transcribeFile } = await Promise.resolve().then(() => __importStar(require('../whisper/transcribe')));
 ...
 */
function readDocumentContent(localPath, maxChars = 8000) {
 ...
 */
function buildDocumentContext(info, content) {
    const parts = [];
    parts.push(`User sent a file: ${info.fileName || 'unnamed'}`);
    if (info.mimeType)
        parts.push(`Type: ${info.mimeType}`);
    if (info.fileSize)
        parts.push(`Size: ${Math.round(info.fileSize / 1024)} KB`);
    if (info.caption)
        parts.push(`Caption: "${info.caption}"`);
    if (content) {
        parts.push(`\nFile contents:\n\`\`\`\n${content}\n\`\`\``);
    }
    return parts.join('\n');
 ...
 */
function buildMediaContext(info, extras) {
    switch (info.type) {
        case 'photo':
